"""Parser per cedolini Zucchetti v2 (header at page2) - 2025/2026."""
...
def detect_zucchetti2(pdf_path: str, full_text: str) -> bool:
    """Detect Zucchetti2 format."""
    if "Zucchetti spa, Autorizzazione Inail n. 299 del 15/01/2009" in full_text:
        return True
    return False


register_parser("zucchetti2", detect_zucchetti2, lambda path: parse_zucchetti2(path))
...
def _pdn(s: str) -> Decimal:
    ''' Parse decimal number wrapper to shorten function name '''
...
def _extract_num_after_label(text: str, label: str) -> Decimal:
    # search for label followed by a number    
    raw_re_grp_label = rf'(?P<label>{re.escape(label)})\s'
    raw_re_grp_num = r'(?P<num>-?\d[\d.]*,\d+)'
    expression = re.compile(raw_re_grp_label + raw_re_grp_num)
    # print(expression.pattern)
    found = expression.search(text)
    if not found or found.group("label") != label:
        return ZERO
    else:
        num = found.group("num")
        return _pdn(num)
...
def parse_zucchetti2(pdf_path: str) -> Cedolino:
    """Parse a Zucchetti2 cedolino PDF."""
    ced = Cedolino(file_path=pdf_path, formato="zucchetti2")
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page in pdf.pages:
            # a single word example:
            # {
            #   'text': 'AZIENDA', 
            #   'x0': 30.3733, 
            #   'x1': 51.755860000000006, 
            #   'top': 21.932800000000043, 
            #   'doctop': 21.932800000000043, 
            #   'bottom': 27.932800000000043, 
            #   'upright': True, 
            #   'height': 6.0, 
            #   'width': 21.382560000000005, 
            #   'direction': 'ltr'
            # }
    ...
    # Parse header, in this format "zucchetti2" header is on page 2
    lines = lines_p1 + lines_p2
    _parse_header(ced, lines)

    # Contratto: doesn't exist in this format, default to Metalmeccanico
    if not ced.contratto:
        ced.contratto = 'Metalmeccanico'

    # Parse salary components
    _parse_salary(ced, lines)

    # Parse voci from all pages
    _parse_voci(ced, lines)
    if lines_p2:
...
def _parse_header(ced: Cedolino, lines: list[dict]):
    ...
    Zucchetti2 PDFs have a structured header on page2 with label rows followed by value rows:
    ...
    for i, line in enumerate(lines):
        ...
        # Livello: "IMP 1 Livello" or "Impiegato 1 Livello" or "IMP Livello C3"
        if "Livello" in text:
            match = re.search(r"(?P<qualifica>IMP)\s+(?P<livello>Livello\s+[A-Z]\d+)", text)
            if match:
                ced.qualifica = "Impiegato" if match.group("qualifica") == "IMP" else match.group("qualifica")
                ced.livello = match.group("livello")
            continue
...
def _parse_totali(ced: Cedolino, lines: list[dict], full_text: str):
    ...
    for i, line in enumerate(lines):
        ...
        if "ARROTONDAMENTO" in text:
            ...
            ced.totali.arrotondamento = _extract_num_after_label(text, "ARROTONDAMENTO")
...
def _add_contribution(ced: Cedolino, desc: str, voce: VoceItem):
    ...
    # In Zucchetti2, the contribution line has: code desc imponibile percentage amount
